    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['Home'])) {
        ...
        if(isset($_POST['Login'])) {
            ...
            foreach($content as $line) {
                $Date = explode(',', $line);
                if(trim($_POST['username']) == trim($Date[0]) && trim($_POST['password']) == trim($Date[1])) {
                    $_SESSION['UserName'] = $Date[0];
                    $loggedin = true;
                    echo "You are logged in";
                    break;
                } elseif(trim($_POST['username']) == trim($Date[0]) && trim($_POST['password']) != trim($Date[1])) {
